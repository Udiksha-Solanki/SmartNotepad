...
class Notepad:
    ...
    def open_file(self, event=None):
        ...
        try:
            self.msg, self.base = self.file_controller.read_file(self.url)
    ...
    def say_something(self):
        try:
            self.takeAudio = self.file_controller.takeQuery()
        ...
        #except RequestError as reqe:
            #messagebox.showerror("Request Error", "Please check your internet connection")
            #print(traceback.format_exc())
        #except UnknownValueError as unkwe:
            #messagebox.showerror("UnknownValue Error", "Speech unrecognizable")
            #print(traceback.format_exc())
        #except WaitTimeoutError as wte:
            #messagebox.showerror("WaitTimeout Error", "Time limit exceeded")
            #print(traceback.format_exc())
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            for values in self.files.keys():
                self.f_name=values
                file_owner=self.files.get(self.f_name)[0]
